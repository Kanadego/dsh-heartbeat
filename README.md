...
| 插件版本 | 适配的 DSH | 说明 |
|---|---|---|
| **v1.7.1**（当前） | ≥ `0.1.2-rc.1` | 修复 `0.1.7` 下「心跳模式」预设丢失：预设改为声明式注册，不再依赖文件系统用户根 |
| v1.7.0 | ≥ `0.1.2-rc.1`（在 `0.1.7-rc.2` 上验收） | 适配 0.1.7 的接口换代（设置接口 / 客户端服务 / 节律配置自持 / 会话消息源）；新增 psy 分区开关与节省 token 模式 |
...
改为在插件的安装清单里**声明式**注册预设（与官方 standard 预设同机制），重启即注册，「心跳模式」重新出现在 Agent 预设页。`0.1.5` 及更早的宿主仍走原来的文件安装路径，两边都不断。
