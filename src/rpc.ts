...
/** Session titles from the host's projection cache (~/.dsh/storages/
 * session_projcache.json): each session key carries rows/title.val once the
 * title service has run. Titles are host data — the client card merges them. */
    ...
    const walk = (node: unknown): void => {
      ...
      for (const [key, value] of Object.entries(node as Record<string, unknown>)) {
        if (key.startsWith('session-') && value && typeof value === 'object') {
          ...
          const rows = (value as { rows?: { title?: { val?: unknown } } }).rows;
          const t = rows?.title ?? (value as { title?: { val?: unknown } }).title;
